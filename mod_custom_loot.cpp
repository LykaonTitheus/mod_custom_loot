...
private:
    void RewardPlayer(Player* player, uint32 bossEntry, std::string bossName)
    {
        ...
        // 2. TOKEN RECOVERY: If they don't have the token (in bags or bank), give/mail it.
        if (!player->HasItemCount(ITEM_LOOT_TOKEN, 1, true))
        {
            ...
            if (msg == EQUIP_ERR_OK)
            {
                ...
                ChatHandler(player->GetSession()).SendSysMessage("Boss Reward Token received/recovered!");
            }
            else
            {
                // Bags full? Send it via mail.
                CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
                MailDraft draft("Lost Token Recovered", "Replacement token for defeating " + bossName + ".");
                if (Item* item = Item::CreateItem(itemId, 1, player))
                {
                    ...
                    player->GetSession()->SendAreaTriggerMessage("Bags full! Your Reward Token was sent by mail.");
                }
            }
        }
        else
        {
            // Just a standard notification if they already have the token
            std::string message = "Boss defeated: " + bossName + "! Reward stored.";
...
public:
    ...
    bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/, uint32 action) override
    {
        ...
        {
            // NOTE: The lines to destroy ITEM_LOOT_TOKEN have been removed.
            // The item is now permanent.
