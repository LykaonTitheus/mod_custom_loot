...
public:
    ...
    void OnPlayerCreatureKill(Player* killer, Creature* killed) override
    {
        ...
        const CreatureTemplate* cTemplate = killed->GetCreatureTemplate();
        Map* map = killed->GetMap();

        bool isBoss = false;
        if (cTemplate->rank >= 3 || killed->isWorldBoss())
            isBoss = true;
        else if (map->IsDungeon() || map->IsRaid())
        {
            if (LootTemplates_Creature.GetLootFor(bossEntry))
                if (cTemplate->rank >= 1)
                    isBoss = true;
        }

        if (bossEntry == 10440) // Baron fix
            isBoss = true;
